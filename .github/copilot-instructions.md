...
Do not expect CLI commands for question answering, memo writing, thesis challenge, assumption selection, or recommendations. Discovery and agent-harness commands write auditable proposal artifacts; the agent owns judgment and interpretation.
...
## Workflow For Discovery And Agent Reviews

Use `investor discovery` to maintain a deterministic triage queue for stock ideas. Discovery may score candidates, write briefs, reject or defer ideas, and propose promotions, but it never mutates holdings and only changes `portfolio/watchlist.json` through `investor discovery promote <TICKER> --approved` after a current clean `analyst_approved` artifact exists.

Use `investor agents run` only when an LLM-backed institutional-pilot review is explicitly useful. Prefer `--provider dry-run` for workflow checks without token spend. For OpenAI-backed runs, set `OPENAI_API_KEY`, control spend with `--limit`, `--ticker`, and `--max-context-chars`, and remember that agent output records `agentSuggestedState`; analyst state is recorded separately with `investor agents approve`.
...
Unsupported claims, stale deterministic data, missing review artifacts, and stale approval source hashes block promotion.
